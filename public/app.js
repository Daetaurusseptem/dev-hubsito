...
function serviceRuntimeStatus(service) { return service.runtimeStatus || (service.ownership === 'managed' ? 'running' : service.ownership === 'external' ? 'external' : 'stopped'); }
function statusLabel(service) {
  return ({ starting: 'INICIANDO', running: 'ACTIVO', external: 'EXTERNO', stopped: 'DETENIDO', failed: 'FALLÓ', unresponsive: 'SIN RESPUESTA' })[serviceRuntimeStatus(service)] || 'DETENIDO';
}
...
function render() {
  ...
  projectsEl.innerHTML = state.projects.map(project => {
    ...
    const hasStarting = project.services.some(service => ['starting', 'unresponsive'].includes(serviceRuntimeStatus(service)));
    ...
    <article class="project-card" style="--accent:${escapeHtml(project.color || '#7c3aed')}">
      ...
      <div class="project-actions"><button class="ghost ${hasStarting ? 'is-busy' : ''}" data-project-action="${allRunning ? 'stop' : 'start'}" data-project="${project.id}" ${hasStarting ? 'disabled aria-busy="true"' : ''}>${hasStarting ? '<span class="button-spinner" aria-hidden="true"></span>Iniciando…' : allRunning ? '■ Detener todo' : '▶ Iniciar todo'}</button><button class="ghost" data-project-settings="${project.id}" title="Configurar proyecto">•••</button></div></header>
      <div class="services">${project.services.map(service => `
        <div class="service"><div class="service-head"><div><div class="service-meta"><i class="dot ${serviceRuntimeStatus(service)}"></i>${technologyMarkup(service)}<strong>${escapeHtml(service.name)}</strong><span class="status-pill ${serviceRuntimeStatus(service)}">${statusLabel(service)}</span>${service.watch ? '<span class="watch-pill">WATCH</span>' : ''}</div><span class="port">${escapeHtml(service.framework || service.kind)} · :${service.port}${service.watch ? ' · recarga automática' : ' · reinicio manual'}${service.ownership === 'external' ? ' · fuera del Hub' : serviceRuntimeStatus(service) === 'unresponsive' ? ' · revisa los logs' : ''}</span></div>
        ...
        ${serviceRuntimeStatus(service) === 'starting' ? `<button class="service-button" data-logs="${service.id}" data-name="${escapeHtml(service.name)}">Logs</button><button class="service-button is-busy" disabled><span class="button-spinner" aria-hidden="true"></span>Iniciando…</button><button class="service-button stop" data-service-action="stop" data-service="${service.id}">Detener</button>` : serviceRuntimeStatus(service) === 'unresponsive' ? `<button class="service-button" data-logs="${service.id}" data-name="${escapeHtml(service.name)}">Logs</button><button class="service-button stop" data-service-action="stop" data-service="${service.id}">Detener</button><button class="service-button" data-service-action="restart" data-service="${service.id}">Reintentar</button>` : serviceRuntimeStatus(service) === 'failed' ? `<button class="service-button" data-logs="${service.id}" data-name="${escapeHtml(service.name)}">Logs</button><button class="service-button start" data-service-action="start" data-service="${service.id}">Reintentar</button>` : service.ownership === 'managed' ? `<button class="service-button" data-logs="${service.id}" data-name="${escapeHtml(service.name)}">Logs</button><button class="service-button stop" data-service-action="stop" data-service="${service.id}">Detener</button><button class="service-button" data-service-action="restart" data-service="${service.id}">Reiniciar</button>` : service.ownership === 'external' && service.kind === 'docker-compose' ? `<button class="service-button stop" data-service-action="stop" data-service="${service.id}">Detener</button><button class="service-button" data-service-action="restart" data-service="${service.id}">Reiniciar</button>` : service.ownership === 'external' ? `<button class="service-button" disabled title="Iniciado fuera del Hub">No administrado</button>` : `<button class="service-button start" data-service-action="start" data-service="${service.id}">Iniciar</button>`}</div></div></div>`).join('')}</div>
    </article>`;
  }).join('');
...
async function action(path, button, { wholeProject = false } = {}) {
  ...
  try { const result = await api(path, { method: 'POST' }); toast(result.message); await new Promise(r => setTimeout(r, 350)); await load(); setWaifuNotice(result.message, path.endsWith('/stop') ? 'sleep' : result.status === 'starting' ? 'focus' : 'success', result.status === 'starting' ? 0 : 3600); }
